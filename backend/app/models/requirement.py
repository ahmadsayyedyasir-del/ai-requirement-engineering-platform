"""
requirement.py — Stores structured, AI-extracted requirements in the database.
...
TABLE STRUCTURE:
  ...
  title             VARCHAR     Short title (≤10 words)
  description       TEXT        Full description
  extra_metadata    JSON        Extra AI-extracted data (e.g., risk impact, likelihood)
  source_input_id   UUID        FK → requirement_inputs.id (traceability)
...
class RequirementCategory(str, enum.Enum):
    ...
    non_functional   — How the system must PERFORM (quality attributes)
                       Example: "The login page shall load in under 2 seconds"
    user_role        — The types of people who use the system
                       Example: "Customer", "Restaurant Owner", "Delivery Rider"
    business_rule    — Policies and constraints from the business domain
                       Example: "Delivery fee is calculated as $2 + $0.50 per km"
    constraint       — Hard technical or regulatory limits
                       Example: "Must be GDPR compliant for EU users"
    assumption       — Things we're assuming to be true (should be validated)
                       Example: "Assumed that payment gateway integration is available"
    risk             — Potential problems that could derail the project
                       Example: "Third-party mapping API may have downtime"
    dependency       — External systems or services this project relies on
...
class RequirementPriority(str, enum.Enum):
    """
    MoSCoW prioritisation framework — the industry standard for requirement priority.
    ...
    must_have    — Without this, the product FAILS. These go in the MVP.
                   Example: "Users must be able to log in"
    should_have  — High value but not critical for launch. Include if possible.
                   Example: "Users should receive email confirmations"
    could_have   — Nice to have. Cut if time/budget is tight.
                   Example: "Users could see their order history graph"
    wont_have    — Explicitly OUT of scope for this version. Not forgotten — deferred.
...
class Requirement(Base, TimestampMixin):
    ...
    # MoSCoW priority — set by the AI during extraction, can be changed manually
    ...
    # Short, searchable title — max 10 words, max 512 characters
    ...
    #   "extra_metadata" is the direct replacement — same purpose, no conflict.
    ...
    # Version counter — incremented each time a user manually edits this requirement.
    ...
    # Soft delete flag — False means the user has "deleted" this requirement.
