"""
requirement_input.py â€” Stores all RAW business input submitted by the user.
...
WHY THIS FILE EXISTS:
  ...
  We keep the raw input separate from the extracted requirements because:
    ...
    2. RE-ANALYSIS: If you want to re-run the AI pipeline with different settings,
       you still have the original input â€” you don't need to re-upload.
...
TABLE STRUCTURE:
  ...
  raw_text         TEXT        For text/transcript inputs â€” the text itself
  file_name        VARCHAR     Original filename (e.g., "requirements_v2.pdf")
  file_path        VARCHAR     Where the file is stored on disk
  file_size_bytes  BIGINT      File size â€” used for validation and display
...
HOW TEXT EXTRACTION WORKS:
  1. User uploads a PDF â†’ file is saved to disk â†’ RequirementInput row is created
...
class InputType(str, enum.Enum):
    ...
    text       â€” User typed or pasted a plain text description directly
    pdf        â€” User uploaded a PDF document (e.g., an existing requirements doc)
    docx       â€” User uploaded a Microsoft Word document
    transcript â€” User pasted a meeting transcript (treated like text but flagged differently)
    voice      â€” (Future) User uploaded a voice recording to be transcribed
    """
    text = "text"
    pdf = "pdf"
    docx = "docx"
...
class RequirementInput(Base, TimestampMixin):
    ...
    # ondelete="CASCADE" â€” if the project is deleted, delete all its inputs too.
    # index=True â€” we often query "all inputs for project X", so an index speeds this up.
    ...
    # Text column has no length limit â€” business descriptions can be very long.
    ...
    # This is what the AI pipeline actually reads â€” not the raw file.
